...
struct DriveActions make_action_lists(String drive_actions, char drive_num){
  ...
  while (step_point < drive_actions.length())  {
    
    String e = strSubs(drive_actions, step_point, "|");
    step_point = step_point +  e.length() + 1;
    if (DEBUG){
            Serial << "RAW Task: " << e << '\n';
            Serial << "drive_actions.length(): " << drive_actions.length() << '\n';
            Serial << "step_point: " << step_point << '\n';
          };
    ...
    if (e.substring(0,1) == "d"){
        ...
        _result.b += String(i)+";"+String(sn)+";"+String(ss)+"|";
        if (DEBUG){
