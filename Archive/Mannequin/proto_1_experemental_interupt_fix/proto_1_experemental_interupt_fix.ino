...
bool driveONEStartDir = false; // true = HIGH (right, in), false = LOW (left, out) // REQ CALIBRATION
// ---------------
...
void FixInter(struct driveInfo _driveInfo){
  //delay(99999999);
  int steps_done_fix = 0;
  while (steps_done_fix < 22){ 
...
void FixInterrupt() {
  ...
  */
     Serial << "FixInt: cd:" << driveOne.currentDir << steps_done <<   "\n";
     driveOne = ChangeDirection(driveOne);
     FixInter(driveOne);
     Serial << "FixInt_end: " << driveOne.currentDir << "\n";
    }
  // }
...
void FixInterruptRight() {
   ...
   */
    int steps_done_fix = 0;
    Serial << "FixIntR_start: " << driveOne.currentDir << ", "  << driveONEWarmingFixedPulsTimeUS << ", " << driveONEWarmingFixedPauseTimeUS << ", " << steps_done_fix <<  "\n";

     //driveOne = ChangeDirection(driveOne);
     driveOne.currentDir = !driveOne.currentDir;
      digitalWrite(driveOne.dirPin, !driveOne.currentDir);
     delay(99999999);
      ...
      while (steps_done_fix < 22){ 
        ...
        steps_done_fix++;
          ...
          Serial << "FixIntR_end: " << driveOne.currentDir <<  ", " << steps_done_fix << "\n";
    }
  // }
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(interruptPinLeft), FixInterrupt, LOW);
  attachInterrupt(digitalPinToInterrupt(interruptPinRight), FixInterrupt, LOW);
