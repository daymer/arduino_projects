...
double driveONEmmPerRota = 3.0; // mm per 1 rotation // REQ CALIBRATION

// -----INPUT----- 
double driveONEdistanceMM = 30.0; // TO GO in mm
double driveONESpeed = 2.0; // Speed in mm/sec
bool driveONEStartDir = false; // true = HIGH (left, in), false = LOW (right, out) // REQ CALIBRATION
// ---------------
...
struct driveInfo GetDriveInfo(int driveNum){
  ...
  if (driveNum == 1){
    ...
    if (PulsCalcMode == 1){
      ...
      _driveInfo.timePulsMS = driveONEFixedPulsTimeMS;
      _driveInfo.selfcorrectionUS = (3.50 * 2) + 50.0;
      _driveInfo.timePauseMS = ((_driveInfo.expectedStepDurationMS - _driveInfo.timePulsMS) * 1000) - _driveInfo.selfcorrectionUS; 
    };
...
struct driveInfo Go(struct driveInfo _driveInfo) {
  ...
  if (_driveInfo.timePulsMS < 5){
    while (1){
      Serial << "Wrong rotation settings! timePuls < 5ms: " << _driveInfo.timePulsMS  << "\n";
      blinkStart(200);
    }
  }

  unsigned int i;
  Serial << "GO: steps: " << _driveInfo.stepsTOgo << ", puls " << _driveInfo.timePulsMS << " ms, pause " << _driveInfo.timePauseMS << " us, selfcorrection: " << _driveInfo.selfcorrectionUS << " us" << "\n";
  long int gt1 = millis();
